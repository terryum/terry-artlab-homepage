...
- `cover_caption` — 원문 figure 캡션 (선택, 번역하지 않고 원문 그대로)
- `cover_thumb` — `"./cover_thumb.webp"` (카드 썸네일용, 선택)
- `card_summary` — 카드/목록 전용 짧은 요약 (ko 85-100자, en 150-180자)
...
### Cover Thumbnail (`cover_thumb.webp`) 생성 규칙
- 용도: 목록 카드 112×112px 썸네일
- `cover.webp`에서 핵심 영역을 1:1로 크롭
- 최소 해상도: 192×192px (2× 대응)
- 텍스트가 아닌 다이어그램/그림 중심 부분을 선택
- 파일명: `cover_thumb.webp`
- **선택 사항**: 없으면 `cover.webp`가 center crop으로 대체됨
...
- [ ] `cover_thumb.webp` 생성 (선택, 없으면 cover.webp로 대체)
- [ ] `paper/<slug>.pdf` 저장
