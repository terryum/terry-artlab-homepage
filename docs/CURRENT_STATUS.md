...
- 현재 단계: Papers 사이드바 outside-container 배치 + 모바일 collapsible 완료. 대기 중.
- 전체 진행도(대략): 100%
...
- [x] 이전 글/다음 글 네비게이션 (3afbc42): TAB_CONFIG.author 기반 저자 그룹별
- [x] Papers 분야별 탐색 사이드바 (9098307): 필터 파이프라인 tab→taxonomy→tag→starred
- [x] **Papers 사이드바 outside-container** (3c9c18b):
  - xl+(≥1280px): absolute outside sidebar, right:calc(100%+1rem), w-44, sticky top-24
  - 모바일/<xl: collapsible 패널 (기본 접힘), 선택 후 자동 닫힘 + 칩 표시
  - Seminal 버튼 숨김 (나중에 재활성화 예정)
...
## 7) 컨텍스트 메모 (다음 세션용)
- Papers 사이드바: xl(≥1280px)에서만 outside sidebar 표시. sticky top-24(96px)
- Seminal 버튼: FilterablePostList.tsx에서 제거됨 (재활성화 시 추가 필요)
