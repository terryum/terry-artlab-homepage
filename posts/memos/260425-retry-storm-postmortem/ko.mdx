---
locale: "ko"
title: "정적 사이트가 5천만 회 두드려진 이유 — retry storm 사후 분석"
subtitle: "정적 자원도 봇 한 대에 무방비"
summary: "한 정적 사이트가 4일간 약 5천만 edge requests를 받았다. figure 경로 stale + 헤드리스 환경에서 자기 루프하는 onerror 패턴 + 외부 자동화의 종료 조건 없는 retry policy가 곱해진 결과였다. 사후 조치 4층(코드 단발화, file-existence validator, egress 무료 호스팅 이전, 알림 셋업)으로 동일 사고 재발 시 금전 손실 0."
card_summary: "5천만 회 두드려진 정적 사이트 — onerror 자기 루프와 헤드리스 자동화의 곱"
terrys_memo: ""
---
...
- [[260419-vercel-quiet-bill|조용한 청구서 — Bot에도 과금하는 Vercel]] — 같은 사건의 비용 구조 측면
- [[260420-one-day-recovery|하루 만에 고친 SkinChat 인프라 부채]] — 묵힌 인프라 부채 한 세션 정비 사례
