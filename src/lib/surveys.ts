 ...
 * group-restricted surveys. Only call after verifying the caller is
 * authorized to see those URLs (e.g., in the detail page after the
 * group auth gate has already run).
 */
 ...
 * client-rendered HTML / RSC payloads. Title / cover / TOC stay public
 * per product policy; only embed_url and external links are redacted
 * because those are the ones that bypass the auth gate.
 */
...
/**
 * Public listing view: surveys.json entries with private URLs stripped
 * for non-public items. Safe to return to any caller, any runtime.
 */
...
/**
 * All surveys (public + private/group). Same sanitization applies —
 * the detail page will re-load the raw meta after the auth gate passes.
 */
 ...
 * for restricted surveys. The caller (survey detail page) is responsible
 * for running requireReadAccess before handing embed_url to the client
 * iframe component.
 */
