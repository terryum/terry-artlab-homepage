---
name: post
description: "Post 생성 파이프라인. 두 갈래: (A) 연구/외부 콘텐츠 요약 (From AI) — arXiv/학술 저널/기술 블로그 URL, KB 노드 추가됨. (B) 직접 쓴 초안 발행 (From Terry/From AI 대화 요약) — Obsidian Drafts의 essays/memos/threads, KB 노드 추가 없음. threads는 ChatGPT 대화 요약 전용(From AI 섹션). virtual/synthesis는 A 계열. --visibility=public(기본)/private로 공개·비공개 선택. 커버 이미지 없을 시 /gemini-3-image-generation 사용."
argument-hint: "<URL | --from=#-N --type=essays|memos|threads | virtual 자연어요청 | synthesis URL1 URL2 ...> [--visibility=public|private|group --group=<g>] [--tags=TAG1,TAG2] [--memo=메모] [--featured]"
---
...
### (B) 직접 쓴 초안 / 대화 요약 경로 (From Terry essays·memos, From AI threads) — KB 노드 추가 **없음**

Obsidian 초안을 공개 포스트로 발행하는 경로. `export-knowledge.mjs` **실행 금지**.

- `--from=<path>` 또는 `--from=#-N` 명시 → **직접 쓴 초안 경로** (Obsidian Drafts에서 발행, `--type=essays|memos|threads` 필수)
- `--type=essays|memos|threads`만 명시 (`--from` 없이 posts/ 폴더 직접 지정) → **직접 쓴 초안 경로**
- `--type=threads` → **From AI/Threads 발행 경로**. `/write` Mode 2(세션 요약) 또는 Mode 5(ChatGPT share URL 요약)가 만든 `content_type: "threads"` 초안이 여기로 넘어옴. Draft frontmatter의 `source`, `source_url`, `source_captured_at`은 meta.json으로 보존.

> **중요**: 이 경로는 "기술 블로그 요약" 경로와 다르다. Terry 본인의 글 (essays/memos)과 AI 대화 요약(threads)은 외부 지식베이스 노드가 아니므로 `terry-papers` 레포에 넣지 않는다.
...
# (B) 직접 쓴 초안 / 대화 요약 경로 (From Terry / From AI)
/post --from=#-3 --type=essays                        → 직접 쓴 초안 → essays 발행
/post --from=#-4 --type=memos                         → 직접 쓴 초안 → memos 발행
/post --from=#-5 --type=threads                       → 대화 요약 초안 → threads 발행 (From AI/Threads)
/post --from=#-5 --type=threads --visibility=private  → 같은 초안을 비공개(Supabase private_content)로만
...
**Research 경로와 From Terry/From AI 경로 모두 지원한다.**

- `--visibility=public` (기본): Git에 파일 저장, `posts/global-index.json` 공개 엔트리 편입, Cloudflare Workers 배포, Obsidian sync
- `--visibility=private`: From Terry/From AI 경로에서 비공개 단일 사용자 발행
  - Supabase `private_content` 테이블에만 저장 (Git/CF 배포/Obsidian sync 스킵)
...
**From Terry/From AI 경로에서의 private/group 처리**:
Step B1~B6(meta.json, MDX 생성)은 동일하되, Step B7 이후가 분기한다 — Git 커밋(B10), Cloudflare 배포(B10.1), sync-obsidian(B10.6), Draft 이동(B10.7)을 모두 스킵하고, 대신 Supabase private_content 업로드 단계로 대체. Research 경로의 group 처리 로직을 재사용한다.
...
## 직접 쓴 초안 / 대화 요약 경로 (From Terry essays·memos, From AI threads)

입력: `[--from=<path>|--from=#-N] [--type=memos|essays|threads] [--visibility=public|private|group --group=<g>] [<slug>]`

Obsidian 초안(Terry가 쓴 essays/memos 또는 AI 대화 요약 threads)을 포스트로 발행하는 경로. **`export-knowledge.mjs` (KB 노드 추가)는 실행하지 않는다** — 이 경로의 콘텐츠는 외부 지식베이스 자산이 아니다.
...
- 예시:
  ...
  /post --from=#-5 --type=threads
  /post --from=#-5 --type=threads --visibility=private
  /post #-4                       # content_type은 draft frontmatter에서 자동
  ```

### `--from` 사용 시 content_type 규칙

- `--from` 사용 시 `--type=memos|essays|threads` 중 하나 지정 필요
  - **memos**: 짧은 생각, 일상 관찰, AI 시대 변화 등 가벼운 글 / 개발·인프라 정리
  - **essays**: 긴 형식의 깊은 사유, AI/Physical AI/미래 변화 등
  - **threads**: AI와의 대화(현재 세션 또는 ChatGPT share URL)를 Terry가 요약한 글. From AI/Threads 섹션에 노출.
- `/write` 또는 `/draft`로 생성한 초안이면 초안의 `content_type` 필드를 자동 사용
...
- **content_type 분기**:
  ...
  - `posts/memos/<slug>/` → `content_type: "memos"` (짧은 메모/생각 / 개발·인프라 정리)
  - `posts/threads/<slug>/` → `content_type: "threads"` (AI 대화 요약, From AI/Threads)
- Slug 규칙: `YYMMDD-<영문-kebab-title>`
- **threads 전용 메타데이터**: Draft frontmatter에서 `source` (예: `"chatgpt"` 또는 `"claude-session"`), `source_url`, `source_captured_at` 세 필드를 추출해 meta.json으로 보존한다. 이 값들은 프론트엔드 `ThreadSourceLine.tsx`가 "Based on a ChatGPT conversation — 원문" 라벨을 렌더링할 때 사용한다. threads인데 `source`가 없으면 사용자에게 확인 후 진행.
...
```json
{
  ...
  "updated_at": "<ISO 8601>",
  "status": "published",
  "content_type": "<essays|memos|threads>",
  "visibility": "<public|private|group>",
  "tags": ["<Essays|Memos|Threads>", "<주제태그1>", "<주제태그2>"],
...
- **태깅 규칙**: 첫 번째 태그는 카테고리 태그 (`"Essays"`, `"Memos"`, `"Threads"` 중 하나) — **대문자** 필수
- **threads 전용 추가 필드** (content_type이 `"threads"`일 때만 포함):
  ```json
  "source": "<chatgpt|claude-session>",
  "source_url": "<원문 URL, chatgpt일 때>",
  "source_captured_at": "<ISO 8601, 파서 실행 시점>"
  ```
...
### Step B10) Git 커밋 + 푸시 (visibility=public일 때만)

`--visibility=private` 또는 `--visibility=group`이면 이 단계와 B10.1, B10.6, B10.7을 **건너뛰고** B10.2 (Supabase 업로드)로 분기한다.
...
### Step B10.1) Cloudflare Workers 배포 (visibility=public일 때 필수)

**Git push만으로는 공개 사이트에 반영되지 않는다** — `posts/index.json`이 빌드 시점 번들에 import되므로(`src/lib/posts.ts` 등), 새 포스트는 Worker 재배포 후에야 `/posts` 목록과 `tab=memos/essays/papers/threads` 필터에 노출된다.
...
### Step B10.2) Supabase 업로드 (visibility=private 또는 group일 때만)

Research 경로의 group 업로드 로직을 재사용한다:
1. MDX 콘텐츠(`ko.mdx`, `en.mdx` 본문) → `private_content` 테이블의 `content_ko`, `content_en` 컬럼
2. meta.json (visibility, allowed_groups 포함) → `meta_json` 컬럼
3. `cover.webp`, 썸네일, OG 이미지 → Supabase Storage `private-covers/{slug}/` 버킷
4. `posts/global-index.json`에 엔트리 추가 (gitignored 범위)
5. Git 커밋 안 함, CF 재배포 불필요 (Supabase 읽기는 런타임에서)
6. `posts/<type>/<slug>/` 로컬 작업 폴더는 업로드 완료 후 삭제 (Git에 흔적 남기지 않기 위함)
7. Draft 원본은 Drafts/ 그대로 유지 (비공개라 vault From Terry/From AI로 이동하지 않음)
...
### Step B10.6) Obsidian Vault Sync (visibility=public일 때만)
- Run: `node scripts/sync-obsidian.mjs --slug=<slug>`
- `scripts/sync-obsidian.mjs`의 `TYPE_TO_FOLDER` 매핑:
...
### Step B10.7) Draft → vault 보존 폴더 이동 (--from + visibility=public일 때 필수)

Draft에서 발행한 경우, 원본 Draft 파일 삭제 후 **원문을 content_type별 vault 폴더에 저장**한다.
...
2. `content_type`에 따라 저장:
   ...
   - `threads` → `~/Codes/personal/terry-obsidian/vault/From AI/Threads/<slug>.md`
3. `global-index.json`에서 음수 draft entry 제거 + `next_private_id`가 삭제된 번호 이후였으면 되돌림

**파일 frontmatter 형식 (essays/memos):**
```markdown
---
doc_id: <공개 ID>
slug: "<slug>"
...
**파일 frontmatter 형식 (threads)** — Draft의 source 필드들을 보존:
```markdown
---
doc_id: <공개 ID>
slug: "<slug>"
source: "<chatgpt|claude-session>"
source_url: "<원문 URL>"         # source=="chatgpt"일 때만
...
- If significant structural/tonal changes were made, update the appropriate guide (**content_type별 분기**):
  - `essays` → `~/Codes/personal/terry-obsidian/vault/write_style_essays.md`
  - `memos` → `~/Codes/personal/terry-obsidian/vault/write_style_memos.md`
  - `threads` → `~/Codes/personal/terry-obsidian/vault/write_style_threads.md`
- Merge new patterns with existing rules; don't let the guide grow too long
- Do NOT mix lessons across guides — each content_type's rules are independent
